...
class FeedbackGiver:
    ...
    def __init__(self):
        ...
        self._prior_feedback = None
        self._prior_right_form_seconds = None
    ...
    def give_feedback(
        ...
        logger.debug(
            ...
            f' feedback "{feedback}" prior_feedback "{self._prior_feedback}"'
        ...
        if rep_completed and exercise_num_steps == 2:
            ...
            if feedback != "Done":
                if self._prior_right_form_seconds != right_form_seconds:
                    self._play_count(right_form_seconds)
                    self._prior_right_form_seconds = right_form_seconds
            elif feedback == "Done" and self._prior_feedback != "Done":
                assert rep_completed
                self._play_advice_step(AdviceSteps.DONE)
        # Get in Frame but only after some seconds
        elif (
            feedback == AdviceSteps.GET_IN_FRAME.value.title and seconds_since_audio > 2
        ):
            self._play_advice_step(AdviceSteps.GET_IN_FRAME)
        # Fix form but only after some seconds
        elif feedback == AdviceSteps.FIX_FORM.value.title and seconds_since_audio > 2:
            # say how to fix the form by mentioning the first bad angle
            logger.warning(f"Angle validation: {angle_validation} angles {angles}")
            bad_angles = [key for key in angle_validation if not angle_validation[key]]
            if bad_angles:
