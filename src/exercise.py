...
class Angle:
    def __init__(self, name: str, recipe: str):
        ...
        self.landmark_indexes = [landmarks[name] for name in recipe.split(",")]
...
class Requirement:
    ...
    def test_req(self, body_value, test_value):
        ...
        raise ValueError(f"Unknown test {self._test}, {body_value} {test_value}")
...
class Pose:
    def __init__(
        self, name: str, requirements: list[dict], all_angles: dict[str, Angle]
    ):
        self.name = name
        self.requirements = [Requirement(defn, all_angles) for defn in requirements]
...
class Step:
    def __init__(self, definition: dict, all_poses: dict[str, Pose]):
        ...
        self.default_target_seconds = definition.get("default_target_seconds", None)
...
class Exercise:
    def __init__(
        self, name: str, all_poses: dict[str, Pose], definition: dict[str, dict]
    ):
        self.name = name
        ...
        self.angles = [
            angle for step in self.steps for angle in step.pose.body_angles()
        ]
        self._indexes_from_angles = self._landmark_indexes_from_all_angles(self.angles)
    ...
    def all_points_in_frame(self, lm_list: list[LandmarkLabel]):
        return lm_list and all(
            lm_list[idx].in_frame for idx in self._indexes_from_angles
        )
    ...
    def _read_exercises(path: str) -> dict[str, "Exercise"]:
        ...
        with open(os.path.join(THIS_DIR, path)) as the_file:
            ...
            all_angles = {
                name: Angle(name, recipe) for name, recipe in data["angles"].items()
            }
            all_poses = {
                name: Pose(name, requirements, all_angles)
                for name, requirements in data["poses"].items()
            }
            return {
                key: Exercise(key, all_poses, val)
                for key, val in data["exercises"].items()
            }

    @staticmethod
    def exercise_names() -> Sequence[str]:
        return list(Exercise._read_exercises(EXERCISE_CONFIGURATION_FILE).keys())
