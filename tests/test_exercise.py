...
class TestExerciseExtractor(TestCase):
    def test_get_list(self):
        self.assertEqual(
            ["Push-up", "Flapping-Cross", "Plank", "Standing-Cross"],
...
class TestPredicates(TestCase):
    def setUp(self) -> None:
        super().setUp()
        self.all_angles = {
            "left_elbow": Angle("left_elbow", "LEFT_SHOULDER,LEFT_ELBOW,LEFT_WRIST"),
            "left_shoulder": Angle(
        ...
        self.all_poses = {
            "pushup up": Pose(
                "pushup up",
                [
                    {"body_angle": "left_elbow", "test": "gt", "angle": 160},
                    {"body_angle": "left_shoulder", "test": "gt", "angle": 40},
                    {"body_angle": "left_hip", "test": "gt", "angle": 160},
                ],
                self.all_angles,
            ),
    ...
    def test_step_is_validated_all_not_good(self):
        angles = {"left_elbow": 80, "left_shoulder": 39, "left_hip": 100}
        step = Step({"name": "step1", "pose": "pushup up"}, all_poses=self.all_poses)
    ...
    def test_step_is_validated_one_not_good(self):
        angles = {"left_elbow": 170, "left_shoulder": 50, "left_hip": 100}
        step = Step({"name": "step1", "pose": "pushup up"}, all_poses=self.all_poses)
        self.assertFalse(step.pose.is_validated(angles))
        self.assertEqual(
    ...
    def test_step_is_validated_all_good(self):
        angles = {"left_elbow": 170, "left_shoulder": 45, "left_hip": 190}
        step = Step({"name": "step1", "pose": "pushup up"}, all_poses=self.all_poses)
    ...
    def test_plank_is_validated_all_good(self):
        angles = {"left_elbow": 92, "left_shoulder": 92, "left_hip": 182}
        step = Step(
            {"name": "step1", "pose": "plank on elbows"}, all_poses=self.all_poses
        )
        self.assertTrue(step.pose.is_validated(angles))
