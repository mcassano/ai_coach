...
def load_frame(path):
    return cv2.imread(os.path.join(THIS_DIR, path), cv2.IMREAD_COLOR)
...
class TestAnnotator(unittest.TestCase):
    ...
    def test_up_down_up_down_good_results(self):
        ...
        for idx in range(0, 5):
            result = annotator.annotate_frame(load_frame(self.pushup_up_file_name))
            self.check_results([idx, "Down", True, 100.0, True], result)

            result = annotator.annotate_frame(load_frame(self.pushup_down_file_name))
